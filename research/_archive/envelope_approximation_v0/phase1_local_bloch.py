...
def compute_band_at_k(
    ...
    """
    Compute frequency at a specific k-point.
    
    Args:
        ms: MPB ModeSolver
        k_point: k-vector
        band_index: Target Band index
    
    Returns:
        target_frequency: ω(k) for the specified band
        all_frequencies: ω(k) for all bands
    """
    ms.k_points = [k_point]
    ms.run()
    # all_freqs is shape (num_k_points, num_bands)
    freqs = ms.all_freqs[0]
    return freqs[band_index - 1], freqs
...
def compute_local_band_data(
    ...
    Returns:
        (omega0, M_inv, v_g, all_omegas):
            omega0: Frequency at k₀ (Target Band)
            M_inv: 2×2 inverse mass tensor (Target Band)
            v_g: 2D group velocity (Target Band)
            all_omegas: Frequencies of all bands at k₀
    """
    # Build geometry
    ...
    # Central frequency
    omega0, all_omegas = compute_band_at_k(ms, k0, geom_config.target_band)
    ...
    # We only need the target band frequency for derivatives (scalar)
    omega_px, _ = compute_band_at_k(ms, k_px, geom_config.target_band)
    omega_mx, _ = compute_band_at_k(ms, k_mx, geom_config.target_band)
    omega_py, _ = compute_band_at_k(ms, k_py, geom_config.target_band)
    omega_my, _ = compute_band_at_k(ms, k_my, geom_config.target_band)
    ...
    omega_pp, _ = compute_band_at_k(ms, k_pp, geom_config.target_band)
    omega_pm, _ = compute_band_at_k(ms, k_pm, geom_config.target_band)
    omega_mp, _ = compute_band_at_k(ms, k_mp, geom_config.target_band)
    omega_mm, _ = compute_band_at_k(ms, k_mm, geom_config.target_band)
    ...
    return omega0, M_inv, v_g, all_omegas
...
class LocalBlochCalculator:
    ...
    def __init__(
        ...
        self.v_g_grid = None
        self.omega_ref = None
        self.all_bands_grid = None # Store all bands
    ...
    def compute_reference_band(self, delta_ref: np.ndarray = None):
        ...
        omega0, M_inv, v_g, all_omegas = compute_local_band_data(
            delta_ref,
            self.setup.a1,
            self.setup.a2,
            self.geom_config,
        ...
        print(f"  Full Spectrum at k0: {all_omegas}")
    ...
    def compute_grid(self, subsample: int = 4):
        ...
        # We don't know the number of bands yet, but we can infer it from a test run or dynamic list
        # We'll use a list of lists/arrays and convert later, or initialize after 1st iteration
        self.all_bands_grid = [[None for _ in range(Ny_sub)] for _ in range(Nx_sub)]
        ...
        for i in range(Nx_sub):
            for j in range(Ny_sub):
                ...
                # Compute band data
                omega, M_inv, v_g, all_omegas = compute_local_band_data(
                    delta_frac,
                    self.setup.a1,
                    self.setup.a2,
                    self.geom_config,
                ...
                self.all_bands_grid[i][j] = all_omegas
                
                count += 1
                if count % 10 == 0:
                    print(f"  Progress: {count}/{total} ({100*count/total:.1f}%)")
        
        # Convert all_bands_grid to numpy array if possible (assuming constant band count)
        try:
            self.all_bands_grid = np.array(self.all_bands_grid)
        except:
            print("Warning: Band count varies across grid, keeping as object array")
    ...
    def save_outputs(self, output_dir: Path):
        ...
        data = {
            ...
            "all_bands_grid": self.all_bands_grid, # Added for Phase 1.5
