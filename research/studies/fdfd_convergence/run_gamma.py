#!/usr/bin/env python3
"""
FDFD TE convergence study at the Γ point (q = 0).
...
def out_path(label, px, f):
    return os.path.join(DATA_DIR,
                        f'fdfd_te_gamma_{label}_res{px}_{freq_tag(f)}.npz')
...
def run_single(m, n, label, px_per_cell, sigma_omega):
    ...
    try:
        ...
        L_op = build_fdfd_operator(
            eps_grid, info, q_vec=Q_GAMMA, polarization='te')
...
def main():
    ...
    ok, fail = 0, 0
    t_start = time.time()
    print(f'Γ-point FDFD TE convergence — {total} runs')
