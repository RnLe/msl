#!/usr/bin/env python3
"""
FDFD TM solve at X point — higher resolution runs at EA center frequencies.
  2° at 16 px/cell  (grid 912²)
  4° at 32 px/cell  (grid 928²)
  8° at 64 px/cell  (grid 896²)
80 modes each.
"""
import gc
import os
...
RUNS = [
    {'m': 14,  'n': 1, 'label': '8deg',  'px': 64, 'sigma_omega': 0.240574},
    {'m': 29,  'n': 1, 'label': '4deg',  'px': 32, 'sigma_omega': 0.241110},
    {'m': 57,  'n': 1, 'label': '2deg',  'px': 16, 'sigma_omega': 0.240887},
]
...
def run_one(m, n, label, px_per_cell, sigma_omega):
...
def main():
    print(f'FDFD TM X-point — high-res EA-centered, {N_MODES} modes')
    for i, r in enumerate(RUNS):
        print(f'\n[{i+1}/{len(RUNS)}] {r["label"]} @ {r["px"]} px/cell')
