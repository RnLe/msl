#!/usr/bin/env python3
"""
Compact spectrum overview for the quarter-Gamma-M TM convergence study (hex rods).

One horizontal panel per angle, resolution lanes with
vertical-line eigenvalues around the target frequency.
"""
import os
import glob
import re
...
# ── EA config ───────────────────────────────────────────────────────
EA_FILES = {}  # Will be populated if EA data exists
EA_LANE_IDX = len(RESOLUTIONS)  # index 4


def discover_ea():
    """Look for EA .npz files in DATA_DIR matching qGM pattern."""
    for fpath in sorted(glob.glob(os.path.join(DATA_DIR, 'qGM_commensurate_*.npz'))):
...
def load_fdfd():
    ...
    files = sorted(glob.glob(os.path.join(DATA_DIR, 'fdfd_tm_quartergm_*.npz')))
    data = {}
    for fpath in files:
        m = re.match(r'fdfd_tm_quartergm_(\w+deg)_res(\d+)_f\w+\.npz',
...
def load_ea():
    ...
    discover_ea()
    data = {}
    for angle, fpath in EA_FILES.items():
        d = np.load(fpath, allow_pickle=True)
...
def main():
    ...
    for row, angle in enumerate(ANGLES_ORDERED):
        ...
        # Figure out which lanes are active (FDFD + EA)
        lane_active = [res in fdfd.get(angle, {}) for res in RESOLUTIONS]
        lane_active.append(angle in ea)
    ...
    fig.suptitle(
        r'TM eigenfrequencies at $\frac{1}{4}\Gamma\!M$  —  hex rods $\varepsilon{=}12$, $r/a{=}0.22$',
        fontsize=12, y=1.0)

    out = os.path.join(FIG_DIR, 'quartergm_tm_hex_spectrum_compact.svg')
    fig.savefig(out, bbox_inches='tight')
    out_png = os.path.join(FIG_DIR, 'quartergm_tm_hex_spectrum_compact.png')
