#!/usr/bin/env python3
"""
FDFD TM solve at X point at the EA reference frequency (f_ref ≈ 0.2443).
Resolution 8 px/cell, 100 modes, all 4 angles.
"""
import gc
import os
import resource
...
# EA reference frequency from the envelope approximation data
SIGMA_OMEGA = 0.244282856911107
N_MODES     = 100
PX_PER_CELL = 8
...
def run_angle(m, n, label):
    fname = f'fdfd_tm_x_{label}_res{PX_PER_CELL}_fEA.npz'
...
def main():
    print(f'FDFD TM X-point at EA freq={SIGMA_OMEGA:.6f}, res={PX_PER_CELL}, '
          f'{N_MODES} modes')
    for i, a in enumerate(ANGLES):
        print(f'\n[{i+1}/{len(ANGLES)}] {a["label"]}')
        run_angle(a['m'], a['n'], a['label'])
    print('\nDone.')
