#!/usr/bin/env python3
"""Corrected MPB TM solve at X point for 8° angle, 100 bands, 64 px/cell."""
import os, time
...
# X point in fractional reciprocal coords: (0.5, 0) of superlattice
k_x = mp.Vector3(0.5, 0, 0)

print(f'8deg X-point: res={mpb_resolution}, {n_bands} bands, {len(geometry)} rods')
print(f'k-point: {k_x}')
...
DATA_DIR = os.path.join(os.path.dirname(__file__), 'data_x_tm_mpb_corrected')
os.makedirs(DATA_DIR, exist_ok=True)
np.savez(os.path.join(DATA_DIR, 'mpb_tm_x_8deg_res64_100bands.npz'),
