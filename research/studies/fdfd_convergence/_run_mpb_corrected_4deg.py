#!/usr/bin/env python3
"""One-shot: corrected MPB solve for 4° angle, 20 bands, 64 px/cell."""
...
print(f'4deg: res={mpb_resolution} (grid {mpb_resolution}x{mpb_resolution}), '
      f'{n_bands} bands, {len(geometry)} rods, L_super={L_super:.2f}')
print(f'Effective px/cell = {mpb_resolution/L_super:.1f}')
...
print(f'Peak RSS: {rss_after:.0f} MB  (before solve: {rss_before:.0f} MB)')
print(f'Bands: {len(freqs_raw)}')
print(f'Freq range (c/a): [{freqs[0]:.6f}, {freqs[-1]:.6f}]')

DATA_DIR = os.path.join(os.path.dirname(__file__), 'data_gamma_tm_mpb_corrected')
os.makedirs(DATA_DIR, exist_ok=True)
np.savez(os.path.join(DATA_DIR, 'mpb_tm_gamma_4deg_res64_20bands.npz'),
