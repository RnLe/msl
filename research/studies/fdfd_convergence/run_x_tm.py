#!/usr/bin/env python3
"""
FDFD TM convergence study at the X point (q = [π, 0]).

Same crystal & angles as the Γ-point study, but q_vec = (π, 0).
...
def out_path(label, px, f):
    return os.path.join(DATA_DIR,
                        f'fdfd_tm_x_{label}_res{px}_{freq_tag(f)}.npz')
...
def main():
    ...
    ok, fail = 0, 0
    t_start = time.time()
    print(f'X-point FDFD TM convergence — {total} runs')
