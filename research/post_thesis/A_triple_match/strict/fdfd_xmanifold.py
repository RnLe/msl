...
Usage: fdfd_xmanifold.py <m> <px> <sigma_omega> <nmodes> <out.npz> [r2] [r1]
       (r2,r1 optional trailing args; default 0.10,0.20 -> frozen candidate)
"""
import os
import sys
import time
...
r2 = float(sys.argv[6]) if len(sys.argv) > 6 else 0.10
r1 = float(sys.argv[7]) if len(sys.argv) > 7 else 0.20
eps2 = float(sys.argv[8]) if len(sys.argv) > 8 else 8.9   # layer-2 contrast knob
n = 1; THR = 0.5; R_CUT = 0.45
N = px * round((m * m + n * n) ** 0.5)
eps, info = build_bilayer_eps_asym(m, n, r1, r2, 8.9, eps2, 1.0, N, N, 8, "centered")
