...
RUNS = [
    ...
    # Jul 7 additions (post RAM raise to 47 GB):
    {'m': 29, 'n': 1, 'label': '4deg', 'px': 16},   # 4 deg edge column
    {'m': 29, 'n': 1, 'label': '4deg', 'px': 32},
    {'m': 57, 'n': 1, 'label': '2deg', 'px': 48},   # sharpen 2 deg drift bar
    # 4 deg true-edge hunt: sigma=0.2270 40-mode ball bottomed out at
    # 0.224134 (px32) without enumerating the edge; go deeper with more modes
    {'m': 29, 'n': 1, 'label': '4deg', 'px': 16, 'sigma': 0.2225, 'n_modes': 60, 'tag': 'DEEP'},
    {'m': 29, 'n': 1, 'label': '4deg', 'px': 32, 'sigma': 0.2225, 'n_modes': 60, 'tag': 'DEEP'},
    {'m': 29, 'n': 1, 'label': '4deg', 'px': 48, 'sigma': 0.2225, 'n_modes': 60, 'tag': 'DEEP'},
]

for r in RUNS:
    m, n, label, px = r['m'], r['n'], r['label'], r['px']
    sigma_omega = r.get('sigma', SIGMA_OMEGA)
    n_modes = r.get('n_modes', N_MODES)
    tag = r.get('tag', '')
    fname = f'fdfd_tm_x_{label}_res{px}_fBOTTOM{tag}.npz'
    ...
    n_dof = L_op.shape[0]
    print(f'{label} res={px}{tag}: DOF={n_dof:,} factorizing...', flush=True)
