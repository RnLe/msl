"""
Phase 0 (MPB): Monolayer Candidate Search & Scoring — V3 Multi-Band Pipeline
...
Output is compatible with phasesV3/phase1_mpb_v3.py
"""
...
def compute_band_subspace_indices(
    ...
    """
    Compute the band indices for multi-band subspace (index-based selection).
    ...
    NOTE: This is index-based selection. For frequency-based selection, use
    compute_band_subspace_by_frequency() instead.
...
def compute_band_subspace_by_frequency(
    target_band: int,
    band_frequencies: np.ndarray,
    delta_omega_subspace: float,
    delta_omega_extra: float,
) -> Dict[str, Any]:
    """
    Compute band indices for multi-band subspace using frequency-based selection.
    
    This is more physically meaningful than index-based selection because it
    ensures bands within a given frequency window are included, regardless of
    how many bands happen to fall in that range.
    
    Args:
        target_band: Central band index (0-based)
        band_frequencies: Array of band frequencies (N_bands,)
        delta_omega_subspace: Include bands within ±delta_omega of target in subspace
        delta_omega_extra: Include additional bands within ±delta_omega_extra for Born-Huang
    
    Returns:
        Dict with same structure as compute_band_subspace_indices()
    """
    omega_target = band_frequencies[target_band]
    total_bands = len(band_frequencies)
    
    # Find subspace bands (within delta_omega_subspace of target)
    omega_sub_low = omega_target - delta_omega_subspace
    omega_sub_high = omega_target + delta_omega_subspace
    subspace_bands = [
        i for i in range(total_bands) 
        if omega_sub_low <= band_frequencies[i] <= omega_sub_high
    ]
    
    # Ensure target is included
    if target_band not in subspace_bands:
        subspace_bands.append(target_band)
        subspace_bands.sort()
    
    # Find extra bands (within delta_omega_extra but outside subspace)
    omega_ext_low = omega_target - delta_omega_extra
    omega_ext_high = omega_target + delta_omega_extra
    
    extra_bands_below = [
        i for i in range(total_bands)
        if omega_ext_low <= band_frequencies[i] < omega_sub_low
    ]
    extra_bands_above = [
        i for i in range(total_bands)
        if omega_sub_high < band_frequencies[i] <= omega_ext_high
    ]
    ...
    return {
        ...
        'target_band': target_band,
        'target_index_in_subspace': subspace_bands.index(target_band) if target_band in subspace_bands else -1,
        'selection_method': 'frequency_based',
        'delta_omega_subspace': delta_omega_subspace,
        'delta_omega_extra': delta_omega_extra,
...
def ensure_run_dir(config: Dict) -> Path:
    ...
    run_name = config.get('run_name', 'mpb_v3')
...
def run_phase0_library_v3(config_path: str, max_bands: Optional[int] = None):
    ...
    """
    log("=" * 70)
    log("Phase 0 (MPB): Candidate Search & Scoring — V3 Multi-Band Pipeline")
    log("=" * 70)
    
    config = load_yaml(config_path)
    log(f"\nLoaded configuration from: {config_path}")
    log(f"Run name: {config.get('run_name', 'mpb_v3')}")
    ...
    target_hs_point = config.get('high_symmetry_point', 'Gamma')
    if target_hs_point == 'Gamma':
        target_hs_point = 'Γ'
        
    lowest_band_mode = config.get('lowest_band_mode', False)
    ...
    log(f"  High-symmetry point: {target_hs_point}")
    log(f"  Lowest Band Mode: {lowest_band_mode}")
        ...
        for r_over_a in r_over_a_list:
            for eps_bg in eps_bg_list:
                ...
                for k_label, k_vec in hs_points:
                    ...
                    # Filter by requested high-symmetry point
                    if target_hs_point != 'all' and k_label != target_hs_point:
                        continue
                    ...
                    for band_index in target_bands:
                        ...
                        if lowest_band_mode:
                            # Override S_total for lowest band mode
                            # Score = -omega + w_gap * gap
                            # We want to minimize omega (maximize -omega) and maximize gap
                            w_gap = config.get('w_gap', 1.1)
                            row['S_total'] = -row['omega0'] + w_gap * row['gap_min']
                            row['score_method'] = 'lowest_band'
    ...
    log("\n" + "=" * 70)
    log("Phase 0 (MPB) — V3 Multi-Band Pipeline complete!")
    log("=" * 70)
    log(f"\nNext step: Run Phase 1 with:")
    log(f"  python phasesV3/phase1_mpb_v3.py auto")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Phase 0 (MPB V3): Multi-Band Candidate Search using band library",
...
"""
    ...
    parser.add_argument(
        ...
        help="Path to configuration YAML (default: configsV3/phase0_mpb.yaml)"
    )
    parser.add_argument(
    ...
    default_config = Path(__file__).parent.parent / "configsV3" / "phase0_mpb.yaml"
