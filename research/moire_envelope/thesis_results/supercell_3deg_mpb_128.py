"""
MPB supercell eigenvalues: (38,1) ~ 3.01°, 128 px/cell, 100 bands.

Canonical square lattice: r/a=0.2, eps_rod=8.9, eps_bg=1.0, TM polarization.
Saves results to supercell_3deg_mpb_128.npz for later FDFD comparison.

Expected: ~10 min, ~15 GB RAM.
"""
...
EPS_ROD = 8.9
EPS_BG = 1.0
RES = 128          # px per unit cell (target)
PX_PER_CELL = 64   # start with 64 to check feasibility; bump to 128 if RAM allows
N_MODES = 100

L1 = np.array([M_IDX, N_IDX], dtype=float)
L2 = np.array([-N_IDX, M_IDX], dtype=float)
L_SUPER = np.sqrt(L1 @ L1)  # ~38.013
N_CELLS = M_IDX**2 + N_IDX**2  # 1445
...
print(f"Resolution: {PX_PER_CELL} px/cell → grid={grid}x{grid} = {grid**2:,} pixels")
...
mp.verbosity(0)
mpb_resolution = PX_PER_CELL * round(L_SUPER)  # px per unit cell, NOT per supercell!
print(f"MPB resolution: {mpb_resolution} (= {PX_PER_CELL} px/cell × {round(L_SUPER)} cells)")
ms = mpb.ModeSolver(
    geometry=geometry,
...
print(f"Solving {N_MODES} TM bands at Γ ... (this may take ~10 min)", flush=True)
t0 = time.time()
ms.run_tm()
t_mpb = time.time() - t0
...
rss_mb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024  # MB on Linux

freqs_mpb_raw = np.array(ms.all_freqs)[0]
freqs_mpb = freqs_mpb_raw / L_SUPER  # convert to a/2πc

print(f"MPB done: {N_MODES} bands in {t_mpb:.1f}s ({t_mpb/60:.1f} min)")
print(f"Peak RSS: {rss_mb:.0f} MB ({rss_mb/1024:.1f} GB)")
print(f"ω range: [{freqs_mpb[0]:.6f}, {freqs_mpb[-1]:.6f}] a/2πc")

# ── Save ────────────────────────────────────────────────────────
outfile = os.path.join(os.path.dirname(__file__), 'supercell_3deg_mpb_128.npz')
np.savez(outfile,
         freqs_mpb=freqs_mpb,
         freqs_mpb_raw=freqs_mpb_raw,
         px_per_cell=PX_PER_CELL, res=mpb_resolution,
         n_modes=N_MODES,
         m=M_IDX, n=N_IDX, L_SUPER=L_SUPER,
         N_cells=N_CELLS, theta_deg=theta_deg,
         grid=grid, t_mpb=t_mpb, rss_mb=rss_mb)
print(f"\nSaved → {outfile}")
print("Contents: freqs_mpb, freqs_mpb_raw, res, n_modes, m, n, L_SUPER, N_cells, theta_deg, grid, t_mpb, rss_mb")
