"""
MPB vs FDFD supercell comparison: (38,1) ~3.01°, res=32, 10 modes.
Quick run to verify agreement before committing to 100 modes.
"""
import os, sys, time
...
print(f"  MPB done: {N_MODES} bands in {t_mpb:.2f}s")
...
eps_grid, info = build_supercell_eps(
    ...
    Nx=N_grid, Ny=N_grid)
t_eps = time.time() - t0
print(f"  ε grid built in {t_eps:.2f}s")
sys.stdout.flush()

t0 = time.time()
L_op = build_fdfd_operator(eps_grid, info, q_vec=np.array([0.0, 0.0]), polarization='tm')
t_build = time.time() - t0
print(f"  Operator: {N_grid**2:,} DOF, nnz={L_op.nnz:,}, built in {t_build:.2f}s")
sys.stdout.flush()

# Free eps_grid to save RAM
del eps_grid

print()
print("=" * 60)
print(f"[3/4] FDFD eigensolver (k={N_MODES}, sigma={SIGMA:.4f})")
print("=" * 60)
print("  Solving (shift-invert, this takes a few minutes)...")
sys.stdout.flush()
...
print(f"  FDFD done: {N_MODES} modes in {t_solve:.2f}s (total {t_eps+t_build+t_solve:.1f}s)")
...
print()
print("=" * 60)
print("[4/4] Save data and plot")
print("=" * 60)

outdir = os.path.dirname(os.path.abspath(__file__))
np.savez(os.path.join(outdir, 'supercell_3deg_10modes_comparison.npz'),
...
for i, f in enumerate(freqs_mpb):
    ax1.plot([0.2, 0.8], [f, f], 'b-', linewidth=1.2, alpha=0.8)
ax1.set_xlim(0, 1)
...
for i, f in enumerate(freqs_fdfd):
    ax2.plot([0.2, 0.8], [f, f], 'r-', linewidth=1.2, alpha=0.8)
ax2.set_xlim(0, 1)
...
mpb_s = np.sort(freqs_mpb[:n_compare])
fdfd_s = np.sort(freqs_fdfd[:n_compare])
ax3.plot(range(n_compare), mpb_s, 'b.-', markersize=6, label='MPB', alpha=0.8)
ax3.plot(range(n_compare), fdfd_s, 'r.-', markersize=6, label='FDFD', alpha=0.8)
...
figpath = os.path.join(outdir, 'fig_supercell_3deg_10modes_ladder.png')
fig.savefig(figpath, dpi=150)
print(f"  Saved: {figpath}")

# Agreement stats
mask = (mpb_s > 0.001) & (fdfd_s > 0.001)
if mask.any():
    rel_err = np.abs(mpb_s[mask] - fdfd_s[mask]) / mpb_s[mask]
    print(f"\nAgreement (excluding zero mode):")
    print(f"  Max  relative error: {rel_err.max():.4%}")
    print(f"  Mean relative error: {rel_err.mean():.4%}")
    print(f"  Modes with <1% error: {np.sum(rel_err < 0.01)}/{len(rel_err)}")
