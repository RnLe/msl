"""
Phase 0: Monolayer Candidate Search & Scoring (Library-Based) — V2 Pipeline

This is part of the V2 pipeline that uses fractional coordinates.
...
def run_phase0_library(config_path: str):
    ...
    """
    print("=" * 70)
    print("Phase 0: Candidate Search & Scoring (Library-Based) — V2 Pipeline")
    print("=" * 70)
    ...
    print(f"  Polarizations: MERGED (TE+TM)" if merge_polarizations else f"  Polarizations: {polarizations}")
    ...
    if merge_polarizations:
        ...
        print(f"         TE and TM bands are MERGED and sorted by frequency at each k-point.")
    ...
    if merge_polarizations and 'dominant_polarization' in df.columns:
        ...
        print(f"  Dominant polarization breakdown: {pol_counts}")
    ...
    print("\n" + "=" * 70)
    print("Phase 0 (Library-Based) — V2 Pipeline complete!")
    if merge_polarizations:
        print("  [MODE] Polarization merging ENABLED (TE+TM bands combined)")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    default_config = Path(__file__).parent.parent / "configsV2" / "phase0_library.yaml"
    ...
    if len(sys.argv) < 2:
        if default_config.exists():
            ...
            print("instead of running MPB calculations. Merges TE and TM polarizations")
            print("for polarization-agnostic envelope approximation.")
            print("\nThis is part of the V2 pipeline using fractional coordinates.")
            print("See README_V2.md for details.")
            sys.exit(1)
    else:
