#!/usr/bin/env python3
"""
R06 Compute: Disorder Robustness (fast, downsampled)
=====================================================
Monte Carlo study of how eigenvalues and localization respond to disorder.

Uses Fourier downsampling 128→NS_D (default 32) for ~20× speedup over
the full-grid version.
...
Part B — Geometric noise (registry perturbation):
    ...
    Implemented as ∂V/∂R · δs (gradient-weighted correlated noise).
...
# ── Configuration ─────────────────────────────────────────────────────────
NS_D = 32                   # downsample grid (32→5120-dim, ~2s/eigsh)
N_REALIZATIONS = 50
NOISE_LEVELS_ONSITE = [0.001, 0.003, 0.01, 0.03, 0.1]   # fraction of V depth
NOISE_LEVELS_GEOM   = [0.001, 0.003, 0.01, 0.03]          # registry noise σ_s/L_m
N_MODES = 20
M_INV_MAX = 20.0


# ── Fourier downsampling (shared with R02) ────────────────────────────────
...
def main():
    print("="*70)
    print(f"R06 Compute: Disorder Robustness (fast, {NS_D}×{NS_D} grid)")
    print("="*70)

    # ── Load Phase 2 data, downsample ─────────────────────────────────────
    ...
    with open(p0_file) as f:
        p0 = json.load(f)
    L_moire   = p0['moire_length']
    theta_deg = p0['theta_deg']

    Ns = NS_D
    N_bands = Lambda_orig.shape[2]

    # Downsample all fields
    print(f"  Downsampling {Ns_orig}→{Ns}...", end=" ", flush=True)
    t0 = time.time()
    Lambda_clean = downsample_field(Lambda_orig, Ns_orig, Ns)
    v_drift_ds   = downsample_field(v_drift, Ns_orig, Ns)
    M_inv_ds     = downsample_field(M_inv, Ns_orig, Ns)
    A_berry_ds   = downsample_field(A_berry, Ns_orig, Ns)
    Phi_BH_ds    = downsample_field(Phi_BH, Ns_orig, Ns)
    print(f"{time.time()-t0:.1f}s")

    dR = L_moire / Ns

    print(f"  Crystal: θ={theta_deg}°, η={eta:.5f}, L_m={L_moire:.2f}a")
    print(f"  Grid: {Ns}×{Ns}×{N_bands} = {Ns*Ns*N_bands}-dim Hamiltonian")
    print(f"  dR = {dR:.4f}a")

    # ── Clean reference solve ─────────────────────────────────────────────
    print("\n  Solving clean Hamiltonian...", flush=True)
    t0 = time.time()
    H_clean = assemble_multiband_hamiltonian(
        Lambda_clean, v_drift_ds, M_inv_ds, A_berry_ds, Phi_BH_ds,
        eta, Ns, Ns, N_bands, dR, dR, B_moire,
    ...
    # Clean IPR and band composition
    ipr_clean = np.zeros(N_MODES)
    dom_weight_clean = np.zeros(N_MODES)
    for m in range(N_MODES):
        psi = evecs_clean[:, m].reshape(Ns, Ns, N_bands)
    ...
    for ni, frac in enumerate(NOISE_LEVELS_ONSITE):
        ...
        print(f"\n  [{ni+1}/{n_noise_A}] σ/ΔV = {frac}, σ_V = {sigma_V:.6e}")
        t0 = time.time()

        for r in range(N_REALIZATIONS):
            Lambda_noisy = Lambda_clean.copy()
            for n in range(N_bands):
                noise = rng.normal(0, sigma_V, size=(Ns, Ns))
                Lambda_noisy[:, :, n, n] += noise

            H_noisy = assemble_multiband_hamiltonian(
                Lambda_noisy, v_drift_ds, M_inv_ds, A_berry_ds, Phi_BH_ds,
                eta, Ns, Ns, N_bands, dR, dR, B_moire,
                M_inv_max_trace=M_INV_MAX
            )
            ...
            for m in range(N_MODES):
                psi = evecs_r[:, m].reshape(Ns, Ns, N_bands)
                dens = np.sum(np.abs(psi)**2, axis=2)
                dens_norm = dens / (dens.sum() + 1e-30)
            ...
            if (r + 1) % 10 == 0:
                dt = time.time() - t0
                rate = dt / (r + 1)
                remaining = rate * (N_REALIZATIONS - r - 1)
                print(f"    r={r+1}/{N_REALIZATIONS}, {dt:.0f}s elapsed, ~{remaining:.0f}s left")
    ...
    # Pre-compute gradient of Lambda diagonal on NS_D grid
    grad_V = np.zeros((Ns, Ns, N_bands, 2))
    for n in range(N_bands):
        V = Lambda_clean[:, :, n, n].real
    ...
    for ni, sigma_s in enumerate(NOISE_LEVELS_GEOM):
        sigma_phys = sigma_s * L_moire
        print(f"\n  [{ni+1}/{n_noise_B}] σ_s/L_m = {sigma_s}, σ_phys = {sigma_phys:.4f}a")
        corr_len = max(3, int(Ns * 0.04))
        t0 = time.time()

        for r in range(N_REALIZATIONS):
            ds1 = build_smooth_noise(Ns, corr_len, rng) * sigma_phys
            ds2 = build_smooth_noise(Ns, corr_len, rng) * sigma_phys
    ...
    # ── Print summary ─────────────────────────────────────────────────────
    print("\n" + "="*70)
    print("Part A summary (on-site noise):")
    for ni, frac in enumerate(NOISE_LEVELS_ONSITE):
        s = summary['part_A']
    ...
    print("\nPart B summary (geometric noise):")
    for ni, sigma_s in enumerate(NOISE_LEVELS_GEOM):
        s = summary['part_B']
