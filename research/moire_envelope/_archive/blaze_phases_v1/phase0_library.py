"""
Phase 0 (BLAZE): Monolayer Candidate Search & Scoring (Library-Based)

This is Phase 0 for the BLAZE pipeline. It uses pre-computed band structures 
from an HDF5 library instead of running MPB calculations.
...
Output is compatible with blaze_phases/phase1_blaze.py
"""
...
def ensure_run_dir(config: Dict) -> Path:
    ...
    Creates: runs/phase0_blaze_YYYYMMDD_HHMMSS/
    """
    output_base = Path(config.get('output_dir', 'runs'))
    run_name = config.get('run_name', 'blaze')
...
def run_phase0_library(config_path: str):
    ...
    This is Phase 0 for the BLAZE pipeline.
    Output is compatible with blaze_phases/phase1_blaze.py
    """
    log("=" * 70)
    log("Phase 0 (BLAZE): Candidate Search & Scoring (Library-Based)")
    log("=" * 70)
    
    # Load configuration
    config = load_yaml(config_path)
    log(f"\nLoaded configuration from: {config_path}")
    log(f"Run name: {config.get('run_name', 'blaze')}")
    ...
    for lattice_type in lattice_types:
        ...
        for polarization in polarizations:
            if polarization not in library.polarizations:
                log(f"  Warning: {polarization} not in library, skipping")
                continue
    ...
    log("\n" + "=" * 70)
    log("Phase 0 (BLAZE) complete!")
    log("=" * 70)
    log(f"\nNext step: Run Phase 1 with:")
    log(f"  python blaze_phases/phase1_blaze.py latest")
...
if __name__ == "__main__":
    # Default config path
    default_config = Path(__file__).parent.parent / "configs" / "phase0_blaze.yaml"
    ...
    if len(sys.argv) < 2:
        if default_config.exists():
            ...
            log("instead of running MPB calculations. Supports both TE and TM polarizations.")
            sys.exit(1)
    else:
