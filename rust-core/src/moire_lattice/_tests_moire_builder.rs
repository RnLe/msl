...
mod tests_moire_builder {
    ...
    use crate::lattice::lattice_construction::{square_lattice, hexagonal_lattice, rectangular_lattice};
    ...
    fn test_builder_new() {
        ...
        assert!(result.is_err(), "Builder without lattice and transformation should fail");
        if let Err(msg) = result {
            assert!(msg.contains("Base lattice not set"), 
                "Error message should mention missing lattice, got: {}", msg);
        }
    }
    ...
    fn test_builder_missing_transformation() {
        ...
        let result = builder.build();
        assert!(result.is_err(), "Builder without transformation should fail");
        if let Err(msg) = result {
            assert!(msg.contains("Transformation not set"), 
                "Error message should mention missing transformation, got: {}", msg);
        }
    }
    ...
    fn test_builder_with_twist_and_scale() {
        ...
        // Check twist angle
        assert!((moire.twist_angle - angle).abs() < 1e-10, 
            "Twist angle = {}, expected = {}", moire.twist_angle, angle);
        
        // Check that lattice_1 is preserved
        assert_eq!(moire.lattice_1.direct, lattice.direct, "Lattice 1 not preserved");
    }
    ...
    fn test_builder_with_tolerance() {
        ...
        assert!(result.is_ok(), "Builder with custom tolerance should succeed");
        let moire = result.unwrap();
    ...
    fn test_builder_with_anisotropic_scale() {
        ...
        if let MoireTransformation::AnisotropicScale { scale_x: sx, scale_y: sy } = moire.transformation {
        ...
        assert!(moire.twist_angle.abs() < 1e-10, 
            "Pure scaling should have zero twist angle, got {}", moire.twist_angle);
    }
    ...
    fn test_builder_with_shear() {
        ...
        if let MoireTransformation::Shear { shear_x: sx, shear_y: sy } = moire.transformation {
    ...
    fn test_builder_with_general_transformation() {
        ...
        assert!(result.is_ok(), "General transformation builder should succeed");
        let moire = result.unwrap();
        ...
        assert!((moire.twist_angle - expected_angle).abs() < 1e-10,
            "Twist angle extraction failed: got {}, expected {}", 
            moire.twist_angle, expected_angle);
    }
    ...
    fn test_builder_zero_scale() {
        ...
        assert!(result.is_err(), "Zero scale should panic due to singular matrix");
    }
    ...
    fn test_builder_negative_scale() {
        ...
        if let Ok(moire) = result {
            ...
            assert!((basis2 - expected).norm() < 1e-10,
                "Negative scale should produce [[-1, 0], [0, -1]], got: {}", basis2);
            ...
            println!("Negative scale failed to build moire: {:?}", result.unwrap_err());
        }
    }
    ...
    fn test_builder_large_angle() {
        ...
        assert!((effective_angle - PI).abs() < 1e-10 || (effective_angle + PI).abs() < 1e-10,
            "Large angle not properly reduced: {}", effective_angle);
    }
    ...
    fn test_builder_identity_transformation() {
        ...
        if let Ok(moire) = result {
            ...
            assert!((moire.lattice_1.direct - moire.lattice_2.direct).norm() < 1e-10,
            ...
            if let Some(indices) = moire.coincidence_indices {
                ...
                assert!(indices == (1, 0, 0, 1) || indices == (-1, 0, 0, -1),
                    "Identity should give trivial coincidence indices, got {:?}", indices);
            }
        } else {
            // If identity transformation fails to build, that might be acceptable
            // depending on the implementation details
            let err = result.unwrap_err();
            println!("Identity transformation failed (might be expected): {}", err);
    ...
    fn test_twisted_bilayer() {
        ...
        if let MoireTransformation::RotationScale { angle: _, scale } = moire.transformation {
            assert!((scale - 1.0).abs() < 1e-10, "Twisted bilayer should have unit scale");
        }
        
        // Check that both lattices have same type
        assert_eq!(moire.lattice_1.bravais, moire.lattice_2.bravais,
    ...
    fn test_twisted_bilayer_zero_angle() {
        ...
        if let Ok(moire) = result {
            assert!(moire.twist_angle.abs() < 1e-10, "Zero twist angle not preserved");
            ...
            let err = result.unwrap_err();
            println!("Zero angle twisted bilayer failed (might be expected): {}", err);
    ...
    */
    ...
    fn test_builder_override_transformation() {
        ...
        assert!(matches!(moire.transformation, MoireTransformation::AnisotropicScale { .. }),
    ...
    fn test_moire_properties() {
        ...
        assert!(result.is_ok(), "Small angle moiré should build successfully");
        let moire = result.unwrap();
        
        // Small twist angle should give large moiré period
        let period_ratio = moire.moire_period_ratio();
        assert!(period_ratio > 10.0, 
            "Small twist angle should give large moiré period, got ratio = {}", period_ratio);
        
        // Moiré cell area should be much larger than original
        assert!(moire.cell_area > moire.lattice_1.cell_area * 100.0,
