...
mod tests_moire2d {
    ...
    use crate::lattice::lattice_construction::{hexagonal_lattice, square_lattice};
    ...
    fn test_rotation_scale_to_matrix() {
        ...
        assert!(
            (mat[(0, 0)] - 2.0 * expected_cos).abs() < 1e-10,
            "Failed: mat[0,0] = {}, expected = {}",
            mat[(0, 0)],
            2.0 * expected_cos
        );
        assert!(
            (mat[(0, 1)] - (-2.0 * expected_sin)).abs() < 1e-10,
            "Failed: mat[0,1] = {}, expected = {}",
            mat[(0, 1)],
            -2.0 * expected_sin
        );
        assert!(
            (mat[(1, 0)] - 2.0 * expected_sin).abs() < 1e-10,
            "Failed: mat[1,0] = {}, expected = {}",
            mat[(1, 0)],
            2.0 * expected_sin
        );
        assert!(
            (mat[(1, 1)] - 2.0 * expected_cos).abs() < 1e-10,
            "Failed: mat[1,1] = {}, expected = {}",
            mat[(1, 1)],
            2.0 * expected_cos
        );
    }
    ...
    fn test_rotation_scale_edge_cases() {
        ...
        assert!(
            ...
            "Zero angle rotation failed"
        );
        assert!(
            mat[(0, 1)].abs() < 1e-10,
            "Zero angle should have no off-diagonal"
        );
        ...
        let mat = transform.to_matrix();
        assert!(
            (mat[(0, 0)] - (-1.0)).abs() < 1e-10,
            "180 degree rotation failed: mat[0,0] = {}",
            mat[(0, 0)]
        );
        assert!(
            mat[(0, 1)].abs() < 1e-10,
            "180 degree rotation has non-zero off-diagonal: {}",
            mat[(0, 1)]
        );
    ...
    fn test_as_lattice2d() {
        ...
        assert_eq!(
            lattice.reciprocal, moire.reciprocal,
            "Reciprocal basis not preserved"
        );
        assert_eq!(lattice.bravais, moire.bravais, "Bravais type not preserved");
        assert_eq!(
            lattice.cell_area, moire.cell_area,
            "Cell area not preserved"
        );
    ...
    fn test_primitive_vectors() {
        ...
        if matches!(
            ...
            assert!(
                dot.abs() < 1e-10,
                "Square lattice vectors should be orthogonal, dot product = {}",
                dot
            );
        }
    }
    ...
    fn test_is_lattice_point() {
        ...
        assert!(
            moire.is_lattice1_point(origin),
            "Origin should be on lattice 1"
        );
        assert!(
            moire.is_lattice2_point(origin),
        ...
        assert!(
            !moire.is_lattice1_point(random_point),
            "Random point should not be on lattice 1"
        );
        assert!(
            !moire.is_lattice2_point(random_point),
    ...
    fn test_is_lattice_point_edge_cases() {
        ...
        assert!(
            moire.is_lattice1_point(close_point),
        ...
        assert!(
            !moire.is_lattice1_point(far_point),
    ...
    fn test_get_stacking_at() {
        ...
        assert_eq!(
            stacking,
            Some("AA".to_string()),
            "Origin should have AA stacking, got {:?}",
            stacking
        );
        ...
        assert_eq!(
            stacking, None,
            "Random point should have no stacking, got {:?}",
            stacking
        );
    ...
    fn test_hexagonal_moire() {
        ...
        assert!(
            a1.norm() > 0.0,
            "Primitive vector should have non-zero length"
        );
        assert!(
            a2.norm() > 0.0,
            "Primitive vector should have non-zero length"
        ...
        assert!(
            (angle - expected_angle).abs() < 0.1 || (angle - PI / 3.0).abs() < 0.1,
    ...
    fn test_commensurate_moire() {
        ...
        assert!(
            comm_moire.is_commensurate,
            "Should be marked as commensurate"
        );
        assert!(
            comm_moire.coincidence_indices.is_some(),
