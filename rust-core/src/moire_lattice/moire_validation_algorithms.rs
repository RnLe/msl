...
pub fn validate_commensurability(
    ...
    for m1 in -max_search..=max_search {
        for m2 in -max_search..=max_search {
            ...
            let error = (v1_frac_in_l2[0] - n1 as f64).abs() + 
                       (v1_frac_in_l2[1] - n2 as f64).abs();
...
pub fn compute_moire_basis(
    ...
    let direct_moire = reciprocal_moire.try_inverse()
...
pub fn analyze_moire_symmetry(moire: &Moire2D) -> Vec<String> {
    ...
    // Hexagonal lattices
    if matches!(moire.lattice_1.bravais, crate::lattice::lattice_bravais_types::Bravais2D::Hexagonal) {
    ...
    // Square lattices
    if matches!(moire.lattice_1.bravais, crate::lattice::lattice_bravais_types::Bravais2D::Square) {
