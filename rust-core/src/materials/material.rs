...
impl PartialEq for Material {
    fn eq(&self, other: &Self) -> bool {
        ...
        (self.epsilon - other.epsilon).norm() < TOLERANCE && (self.mu - other.mu).norm() < TOLERANCE
    }
}
...
impl CommonMaterials {
    ...
    pub fn silicon() -> Material {
        Material::dielectric(Complex64::new(12.25, 0.0), Some("Silicon".to_string()))
    }

    /// Silica glass (epsilon ≈ 2.13, n ≈ 1.46)
    pub fn silica() -> Material {
        Material::dielectric(Complex64::new(2.13, 0.0), Some("Silica".to_string()))
    }

    /// Gallium Arsenide at 1550 nm (epsilon ≈ 11.0, n ≈ 3.32)
    pub fn gaas() -> Material {
        Material::dielectric(Complex64::new(11.0, 0.0), Some("GaAs".to_string()))
    }

    /// Aluminum Oxide (Sapphire) (epsilon ≈ 9.8, n ≈ 1.76)
    pub fn al2o3() -> Material {
        Material::dielectric(Complex64::new(9.8, 0.0), Some("Al2O3".to_string()))
    }
}
...
mod tests {
    ...
    fn test_dielectric_material() {
        let mat = Material::dielectric(Complex64::new(2.25, 0.0), Some("Dielectric".to_string()));
    ...
    fn test_from_refractive_index() {
        let mat =
            Material::from_refractive_index(Complex64::new(1.5, 0.0), Some("Glass".to_string()));
