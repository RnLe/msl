...
pub fn rhombohedral_lattice(a: f64, alpha: f64) -> Lattice3D {
    ...
    let direct = Matrix3::new(
        a, a * cos_alpha, a * cos_alpha,
        0.0, a * sin_alpha, a * (cos_alpha - cos_alpha.powi(2) / sin_alpha.powi(2)).sqrt(),
        0.0, 0.0, a * (1.0 - 3.0 * cos_alpha.powi(2) + 2.0 * cos_alpha.powi(3)).sqrt() / sin_alpha,
    );
    Lattice3D::new(direct, 1e-10)
}
