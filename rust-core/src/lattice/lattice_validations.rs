...
pub fn analyze_bravais_type_2d(lattice: &Lattice2D) -> (Bravais2D, String) {
    ...
    let reason = match bravais_type {
        Bravais2D::Square => {
            format!(
                "Square lattice: a = b = {:.6}, γ = {:.2}° ≈ 90°",
                a, gamma_deg
            )
        }
        Bravais2D::Hexagonal => {
            format!(
                "Hexagonal lattice: a = b = {:.6}, γ = {:.2}° ≈ 60°/120°",
                a, gamma_deg
            )
        }
        Bravais2D::CenteredRectangular => {
            format!(
                "Centered Rectangular lattice: a = {:.6}, b = {:.6} (a ≠ b), γ = {:.2}° ≈ 90°, with centering",
                a, b, gamma_deg
            )
        }
        Bravais2D::Rectangular => {
            format!(
                "Rectangular lattice: a = {:.6}, b = {:.6} (a ≠ b), γ = {:.2}° ≈ 90°",
                a, b, gamma_deg
            )
        }
        Bravais2D::Oblique => {
