...
mod tests_voronoi_cells {
    ...
    fn test_compute_wigner_seitz_cell_2d_square() {
        ...
        if ws_cell.vertices.len() != 4 || (ws_cell.measure - 1.0).abs() > 0.01 {
            eprintln!("DEBUG: Square lattice WS cell has {} vertices", ws_cell.vertices.len());
        ...
        assert_eq!(ws_cell.vertices.len(), 4, "Square WS cell should have 4 vertices");
        assert_eq!(ws_cell.edges.len(), 4, "Square WS cell should have 4 edges");
        assert!((ws_cell.measure - 1.0).abs() < 0.01, "Square WS cell area should be 1.0");

        // Verify vertices are at (±0.5, ±0.5, 0)
        for vertex in &ws_cell.vertices {
            assert!(vertex.z.abs() < TEST_TOLERANCE, "2D vertices should have z=0");
            assert!((vertex.x.abs() - 0.5).abs() < 0.01, "x coordinates should be ±0.5");
            assert!((vertex.y.abs() - 0.5).abs() < 0.01, "y coordinates should be ±0.5");
        }
    }
    ...
    fn test_compute_wigner_seitz_cell_2d_hexagonal() {
        ...
        if ws_cell.vertices.len() != 6 {
            eprintln!("DEBUG: Hexagonal lattice WS cell has {} vertices, expected 6", ws_cell.vertices.len());
            eprintln!("DEBUG: Vertices: {:?}", ws_cell.vertices);
            eprintln!("DEBUG: WS cell area = {}", ws_cell.measure);
        }

        // Hexagonal lattice should have hexagonal WS cell with 6 vertices
        assert_eq!(ws_cell.vertices.len(), 6, "Hexagonal WS cell should have 6 vertices");
        assert_eq!(ws_cell.edges.len(), 6, "Hexagonal WS cell should have 6 edges");
        
        // Area should be sqrt(3)/2 for unit hexagonal lattice
        let expected_area = 0.5 * (3.0_f64).sqrt();
        assert!((ws_cell.measure - expected_area).abs() < 0.01, 
                "Hexagonal WS cell area incorrect: got {}, expected {}", ws_cell.measure, expected_area);
    }
    ...
    fn test_compute_wigner_seitz_cell_2d_rectangular() {
        ...
        if ws_cell.vertices.len() != 4 || (ws_cell.measure - 2.0).abs() > 0.01 {
            eprintln!("DEBUG: Rectangular lattice WS cell has {} vertices", ws_cell.vertices.len());
            eprintln!("DEBUG: WS cell area = {}, expected = 2.0", ws_cell.measure);
            eprintln!("DEBUG: Vertices: {:?}", ws_cell.vertices);
        }

        // Should have rectangular WS cell
        assert_eq!(ws_cell.vertices.len(), 4, "Rectangular WS cell should have 4 vertices");
        assert!((ws_cell.measure - 2.0).abs() < 0.01, "Rectangular WS cell area should be 2.0");
    }
    ...
    fn test_compute_wigner_seitz_cell_2d_oblique() {
        ...
        assert!(ws_cell.vertices.len() >= 4, "Oblique WS cell should have at least 4 vertices");
    ...
    fn test_compute_wigner_seitz_cell_2d_tiny_lattice() {
        ...
        assert!(!ws_cell.vertices.is_empty(), "WS cell should have vertices even for tiny lattice");
        assert!(ws_cell.measure > 0.0, "WS cell area should be positive");
        assert!((ws_cell.measure - scale * scale).abs() < scale * scale * 0.1, 
    ...
    fn test_compute_wigner_seitz_cell_3d_cubic() {
        ...
        if ws_cell.vertices.len() != 8 || (ws_cell.measure - 1.0).abs() > 0.01 {
            eprintln!("DEBUG: Cubic lattice WS cell has {} vertices", ws_cell.vertices.len());
            eprintln!("DEBUG: WS cell volume = {}, expected = 1.0", ws_cell.measure);
            eprintln!("DEBUG: Number of faces = {}", ws_cell.faces.len());
        }

        // Cubic lattice WS cell is a cube
        assert_eq!(ws_cell.vertices.len(), 8, "Cubic WS cell should have 8 vertices");
        assert_eq!(ws_cell.faces.len(), 6, "Cubic WS cell should have 6 faces");
        assert!((ws_cell.measure - 1.0).abs() < 0.01, "Cubic WS cell volume should be 1.0");
    }
    ...
    fn test_compute_wigner_seitz_cell_3d_fcc() {
        ...
        assert!(ws_cell.vertices.len() == 14 || ws_cell.vertices.len() == 8, 
    ...
    fn test_compute_wigner_seitz_cell_3d_extreme_aspect_ratio() {
        ...
        assert!(ws_cell.measure > 0.0, "WS cell volume should be positive");
        assert!((ws_cell.measure - 100.0).abs() < 1.0, "WS cell volume should be ~100");
    }
    ...
    fn test_compute_brillouin_zone_2d_square() {
        ...
        if bz.vertices.len() != 4 {
            eprintln!("DEBUG: Square BZ has {} vertices, expected 4", bz.vertices.len());
        ...
        assert!((bz.measure - expected_area).abs() < 0.1, 
                "Square BZ area incorrect: got {}, expected {}", bz.measure, expected_area);
    }
    ...
    fn test_compute_brillouin_zone_3d_cubic() {
        ...
        assert!((bz.measure - expected_volume).abs() < 0.1, 
                "Cubic BZ volume incorrect: got {}, expected {}", bz.measure, expected_volume);
    }
    ...
    fn test_generate_lattice_points_2d_within_radius() {
        ...
        // Debug info
        if points.len() != 8 {
            eprintln!("DEBUG: Found {} points within radius {}", points.len(), radius);
        ...
        assert_eq!(points.len(), 8, "Should have 8 points within radius sqrt(2)");
        
        // Verify all points are within radius
        for point in &points {
            assert!(point.norm() <= radius + TEST_TOLERANCE, 
                    "Point {:?} outside radius {}", point, radius);
        }
    }
    ...
    fn test_generate_lattice_points_3d_within_radius() {
        ...
        assert_eq!(nearest_points.len(), 6, "Should have 6 points within radius 1.1");
        
        // All nearest neighbors should be at distance 1
        for point in &nearest_points {
            assert!((point.norm() - 1.0).abs() < TEST_TOLERANCE, 
    ...
    fn test_generate_lattice_points_edge_cases() {
        ...
        assert_eq!(points.len(), (21 * 21) - 1, "Shell 10 should have 440 points");
    }
    ...
    fn test_wigner_seitz_reciprocal_consistency() {
        ...
        if (product - expected).abs() > 0.1 {
            eprintln!("DEBUG: WS area * BZ area = {}, expected = {}", product, expected);
            eprintln!("DEBUG: WS area = {}, BZ area = {}", ws_cell.measure, bz.measure);
        }
