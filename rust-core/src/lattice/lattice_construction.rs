use crate::lattice::{Lattice2D, lattice_like_2d::LatticeLike2D};
...
pub fn square_lattice(a: f64) -> Result<Lattice2D, anyhow::Error> {
    ...
    Lattice2D::from_direct_matrix(direct)
}
...
pub fn rotate_lattice_2d(lattice: &Lattice2D, angle: f64) -> Result<Lattice2D, anyhow::Error> {
    ...
    let rotated_direct = rotation_matrix * lattice.direct_basis().base_matrix();
    Lattice2D::from_direct_matrix(rotated_direct)
}
...
pub fn scale_lattice_2d(
    ...
    let scaled_direct = lattice.direct_basis().base_matrix() * scale_factor;
    Lattice2D::from_direct_matrix(scaled_direct)
}
...
pub fn transform_lattice_2d(
    ...
    let transformed_direct = transformation * lattice.direct_basis().base_matrix();
    Lattice2D::from_direct_matrix(transformed_direct)
}
...
pub fn create_supercell_2d(
    ...
    let supercell_direct = lattice.direct_basis().base_matrix() * supercell_matrix;
    Lattice2D::from_direct_matrix(supercell_direct)
}
