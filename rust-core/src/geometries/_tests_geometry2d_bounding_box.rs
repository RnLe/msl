...
mod tests {
    ...
    fn test_new_with_valid_bounds() {
        let bbox = BoundingBox2D::new(Vector2::new(-5.0, -10.0), Vector2::new(5.0, 10.0));
    ...
    #[test]
    #[should_panic(
        expected = "Minimum coordinates must be less than or equal to maximum coordinates"
    )]
    #[cfg(debug_assertions)]
    fn test_new_with_invalid_bounds() {
        // This should panic in debug mode
        let _bbox = BoundingBox2D::new(Vector2::new(5.0, 10.0), Vector2::new(-5.0, -10.0));
    }

    #[test]
    fn test_from_center_size_edge_cases() {
        // Test with zero size
        let bbox = BoundingBox2D::from_center_size(Vector2::new(3.0, 4.0), Vector2::new(0.0, 0.0));
        ...
        let bbox2 =
            BoundingBox2D::from_center_size(Vector2::new(-3.0, -4.0), Vector2::new(2.0, 2.0));
    ...
    fn test_from_points_edge_cases() {
        ...
        assert_eq!(
            bbox2.height(),
            0.0,
            "Height should be 0 for collinear horizontal points"
        );
        ...
        assert!(
            bbox3.is_empty(),
            "Bounding box of identical points should be empty"
        );
    }

    #[test]
    fn test_contains_edge_cases() {
        let bbox = BoundingBox2D::new(Vector2::new(-1.0, -1.0), Vector2::new(1.0, 1.0));

        // Test exact boundaries
        assert!(
            bbox.contains(Vector2::new(-1.0, -1.0)),
            "Should contain min corner"
        );
        assert!(
            bbox.contains(Vector2::new(1.0, 1.0)),
            "Should contain max corner"
        );
        assert!(
            bbox.contains(Vector2::new(-1.0, 1.0)),
        ...
        // Test edges
        assert!(
            bbox.contains(Vector2::new(0.0, -1.0)),
            "Should contain bottom edge point"
        );
        assert!(
            bbox.contains(Vector2::new(0.0, 1.0)),
            "Should contain top edge point"
        );
        assert!(
            bbox.contains(Vector2::new(-1.0, 0.0)),
            "Should contain left edge point"
        );
        assert!(
            bbox.contains(Vector2::new(1.0, 0.0)),
            "Should contain right edge point"
        );

        // Test just outside
        let epsilon = 1e-10;
        assert!(
            !bbox.contains(Vector2::new(-1.0 - epsilon, 0.0)),
            "Should not contain point just outside left"
        );
        assert!(
            !bbox.contains(Vector2::new(1.0 + epsilon, 0.0)),
            "Should not contain point just outside right"
        );
        assert!(
            !bbox.contains(Vector2::new(0.0, -1.0 - epsilon)),
            "Should not contain point just outside bottom"
        );
        assert!(
            !bbox.contains(Vector2::new(0.0, 1.0 + epsilon)),
            "Should not contain point just outside top"
        );
    }

    #[test]
    fn test_contains_box_edge_cases() {
        let outer = BoundingBox2D::new(Vector2::new(-2.0, -2.0), Vector2::new(2.0, 2.0));

        // Test identical boxes
        let identical = BoundingBox2D::new(Vector2::new(-2.0, -2.0), Vector2::new(2.0, 2.0));
        assert!(outer.contains_box(&identical), "Box should contain itself");

        // Test touching edges
        let touching = BoundingBox2D::new(Vector2::new(-2.0, -2.0), Vector2::new(0.0, 0.0));
        assert!(
            outer.contains_box(&touching),
            "Should contain box touching edges"
        );

        // Test slightly outside
        let outside = BoundingBox2D::new(Vector2::new(-2.1, -2.0), Vector2::new(2.0, 2.0));
        assert!(
            !outer.contains_box(&outside),
            "Should not contain box extending outside"
        );
    }
    ...
    fn test_intersects_special_cases() {
        ...
        let corner_touch = BoundingBox2D::new(Vector2::new(2.0, 2.0), Vector2::new(3.0, 3.0));
        assert!(
            bbox1.intersects(&corner_touch),
            "Boxes touching at corner should intersect"
        );

        // Test touching edges
        let edge_touch = BoundingBox2D::new(Vector2::new(2.0, 0.0), Vector2::new(3.0, 2.0));
        assert!(
            bbox1.intersects(&edge_touch),
            "Boxes touching at edge should intersect"
        );

        // Test one box inside another
        let inside = BoundingBox2D::new(Vector2::new(0.5, 0.5), Vector2::new(1.5, 1.5));
        assert!(bbox1.intersects(&inside), "Contained box should intersect");
        assert!(
            inside.intersects(&bbox1),
    ...
    fn test_intersection_edge_cases() {
        ...
        let edge_box = BoundingBox2D::new(Vector2::new(2.0, 0.0), Vector2::new(4.0, 2.0));
        let intersection = bbox1.intersection(&edge_box);
        assert!(
            intersection.is_some(),
            "Edge touching boxes should have intersection"
        );
        let int = intersection.unwrap();
        assert!(
            int.is_empty(),
            "Edge intersection should be empty (zero width)"
        );

        // Test corner intersection
        let corner_box = BoundingBox2D::new(Vector2::new(2.0, 2.0), Vector2::new(4.0, 4.0));
        let corner_int = bbox1.intersection(&corner_box);
        assert!(
            corner_int.is_some(),
            "Corner touching boxes should have intersection"
        );
    ...
    fn test_union_special_cases() {
        ...
        let normal = BoundingBox2D::new(Vector2::new(1.0, 1.0), Vector2::new(3.0, 3.0));
        let empty = BoundingBox2D::new(Vector2::new(5.0, 5.0), Vector2::new(5.0, 5.0));
    ...
    fn test_properties_edge_cases() {
        ...
        assert!(
            empty.aspect_ratio().is_infinite(),
            "Empty box should have infinite aspect ratio"
        );
        assert!(empty.is_empty(), "Should correctly identify as empty");

        // Test zero height box
        let flat = BoundingBox2D::new(Vector2::new(0.0, 5.0), Vector2::new(10.0, 5.0));
        assert_eq!(flat.height(), 0.0, "Flat box should have zero height");
        assert_eq!(flat.width(), 10.0, "Flat box should have correct width");
        assert!(
            flat.aspect_ratio().is_infinite(),
            "Flat box should have infinite aspect ratio"
        );
    ...
    fn test_expand_edge_cases() {
        ...
        assert!(
            over_shrunk.width() <= 0.0,
            "Over-shrunk box should have non-positive width"
        );
    ...
    fn test_numerical_precision() {
        ...
        let tiny = BoundingBox2D::new(Vector2::new(1e-15, 1e-15), Vector2::new(2e-15, 2e-15));
        assert!(tiny.width() > 0.0, "Should handle tiny positive dimensions");

        // Test with very large values
        let huge = BoundingBox2D::new(Vector2::new(-1e15, -1e15), Vector2::new(1e15, 1e15));
        ...
        assert!(
            !near_zero.is_empty(),
            "Near-zero dimensions should not be considered empty"
        );
    }
}
