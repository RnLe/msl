...
use nalgebra::{Vector2, Rotation2, Matrix3};
...
impl Transform2D {
    ...
    pub fn apply_inverse_to_point(&self, point: Vector2<f64>) -> Vector2<f64> {
        assert!(self.scale.x != 0.0 && self.scale.y != 0.0, 
    ...
    pub fn to_matrix(&self) -> Matrix3<f64> {
        ...
        Matrix3::new(
            self.scale.x * cos_r, -self.scale.y * sin_r, self.translation.x,
            self.scale.x * sin_r,  self.scale.y * cos_r, self.translation.y,
            0.0,                   0.0,                  1.0,
        )
    }
