...
impl Geometry2D for Rectangle {
    fn contains_point(&self, point: Vector2<f64>) -> bool {
        ...
        local_point.x.abs() <= self.size.x / 2.0 && local_point.y.abs() <= self.size.y / 2.0
    }
...
impl Geometry2D for Ellipse {
    ...
    fn bounding_box(&self) -> BoundingBox2D {
        ...
        let x_extent =
            ((self.semi_axes.x * cos_r).powi(2) + (self.semi_axes.y * sin_r).powi(2)).sqrt();
        let y_extent =
            ((self.semi_axes.x * sin_r).powi(2) + (self.semi_axes.y * cos_r).powi(2)).sqrt();
...
impl Polygon {
    ...
    pub fn regular(center: Vector2<f64>, radius: f64, sides: usize) -> Self {
        ...
        for i in 0..sides {
            ...
            vertices.push(center + Vector2::new(radius * angle.cos(), radius * angle.sin()));
        }
...
impl CompoundGeometry {
    pub fn new(geometries: Vec<Box<dyn Geometry2D>>) -> Self {
        let center = if geometries.is_empty() {
            ...
            let sum: Vector2<f64> = geometries.iter().map(|g| g.center()).sum();
    ...
    fn update_center(&mut self) {
        if !self.geometries.is_empty() {
            let sum: Vector2<f64> = self.geometries.iter().map(|g| g.center()).sum();
...
impl Geometry2D for CompoundGeometry {
    ...
    fn clone_box(&self) -> Box<dyn Geometry2D> {
        let cloned_geometries = self.geometries.iter().map(|g| g.clone_box()).collect();
