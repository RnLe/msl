...
fn registry_center_to_py(py: Python<'_>, c: &CoreRegistryCenter) -> PyObject {
    ...
    dict.set_item("tau", vec![c.tau.x, c.tau.y, c.tau.z])
        .unwrap();
    dict.set_item("position", vec![c.position.x, c.position.y, c.position.z])
        .unwrap();
    dict.into()
}
...
pub fn py_registry_centers(moire: &PyMoire2D, d0x: f64, d0y: f64) -> PyResult<Vec<PyObject>> {
    ...
    let centers = moire
        .inner
        .registry_centers_monatomic(d0)
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e))?;
    Python::with_gil(|py| {
        Ok(centers
            .iter()
            .map(|c| registry_center_to_py(py, c))
            .collect())
    })
}
...
pub fn py_local_cells_preliminary(moire: &PyMoire2D, d0x: f64, d0y: f64) -> PyResult<PyObject> {
    ...
    let map = moire
        .inner
        .local_bravais_and_basis_from_registries_preliminary(d0)
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e))?;
    Python::with_gil(|py| {
        ...
        for (label, (lat, basis)) in map.into_iter() {
            ...
            let basis_list: Vec<Vec<f64>> =
                basis.into_iter().map(|v| vec![v.x, v.y, v.z]).collect();
...
pub fn py_local_cell_at_point_preliminary(
    ...
    let (label, lat, basis) = moire
        .inner
        .local_bravais_and_basis_at_point_preliminary(point, d0)
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e))?;
