...
impl PyMoireBuilder {
    ...
    fn with_twist_and_scale(&mut self, angle: f64, scale: f64) -> PyResult<()> {
        self.transformation = Some(moire_lattice::moire_lattice::MoireTransformation::RotationScale { angle, scale });
        Ok(())
    }
    
    /// Set an anisotropic scaling transformation
    fn with_anisotropic_scale(&mut self, scale_x: f64, scale_y: f64) -> PyResult<()> {
        self.transformation = Some(moire_lattice::moire_lattice::MoireTransformation::AnisotropicScale { scale_x, scale_y });
        Ok(())
    }
    
    /// Set a shear transformation
    fn with_shear(&mut self, shear_x: f64, shear_y: f64) -> PyResult<()> {
        self.transformation = Some(moire_lattice::moire_lattice::MoireTransformation::Shear { shear_x, shear_y });
        Ok(())
    }
    ...
    fn with_general_transformation(&mut self, matrix: Vec<Vec<f64>>) -> PyResult<()> {
        ...
        self.transformation = Some(moire_lattice::moire_lattice::MoireTransformation::General(mat));
        Ok(())
    }
    ...
    fn build(&self) -> PyResult<PyMoire2D> {
        let lattice = self.lattice.as_ref()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Base lattice not set"))?;
        let transformation = self.transformation.as_ref()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Transformation not set"))?;
