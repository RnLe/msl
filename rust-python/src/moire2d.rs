...
use moire_lattice::moire_lattice::moire2d::{self, Moire2D, MoireTransformation};
...
impl PyMoireTransformation {
    ...
    fn twist(angle: f64) -> Self {
        PyMoireTransformation {
            inner: MoireTransformation::Twist { angle },
        }
    }
    ...
    fn general(matrix: [[f64; 2]; 2]) -> Self {
        let mat = Matrix2::new(matrix[0][0], matrix[1][0], matrix[0][1], matrix[1][1]);
        PyMoireTransformation {
    ...
    fn __repr__(&self) -> String {
        match &self.inner {
            ...
            MoireTransformation::RotationScale { angle, scale } => format!(
                "MoireTransformation.RotationScale(angle={:.4}, scale={:.4})",
                angle, scale
            ),
            MoireTransformation::AnisotropicScale { scale_x, scale_y } => format!(
                "MoireTransformation.AnisotropicScale(scale_x={:.4}, scale_y={:.4})",
                scale_x, scale_y
            ),
            MoireTransformation::Shear { shear_x, shear_y } => format!(
                "MoireTransformation.Shear(shear_x={:.4}, shear_y={:.4})",
                shear_x, shear_y
            ),
...
impl PyMoire2D {
    ...
    fn from_transformation(
        base_lattice: &PyLattice2D,
        transformation: &PyMoireTransformation,
    ) -> PyResult<Self> {
        moire2d::from_transformation(&base_lattice.inner, transformation.inner.clone())
            .map(|inner| PyMoire2D { inner })
    ...
    fn direct_basis(&self) -> PyBaseMatrixDirect {
        PyBaseMatrixDirect {
            inner: self.inner.direct_basis().clone(),
        }
    }

    /// Get the reciprocal space basis matrix
    fn reciprocal_basis(&self) -> PyBaseMatrixReciprocal {
        PyBaseMatrixReciprocal {
            inner: self.inner.reciprocal_basis().clone(),
        }
    }
    ...
    fn direct_bravais(&self) -> PyBravais2D {
        PyBravais2D {
            inner: self.inner.direct_bravais(),
        }
    }
    ...
    fn reciprocal_bravais(&self) -> PyBravais2D {
        PyBravais2D {
            inner: self.inner.reciprocal_bravais(),
        }
    }
    ...
    fn wigner_seitz(&self) -> PyPolyhedron {
        PyPolyhedron {
            inner: self.inner.wigner_seitz().clone(),
        }
    }
    ...
    fn direct_high_symmetry(&self) -> PyHighSymmetryData {
        PyHighSymmetryData {
            inner: self.inner.direct_high_symmetry().clone(),
        }
    }
    ...
    fn reciprocal_high_symmetry(&self) -> PyHighSymmetryData {
        PyHighSymmetryData {
            inner: self.inner.reciprocal_high_symmetry().clone(),
        }
    }
    ...
    fn __repr__(&self) -> String {
        format!(
            "Moire2D(direct_bravais={}, a={:.4}, b={:.4})",
